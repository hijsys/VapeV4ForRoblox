...
function mainapi:CreateGUI()
	...
	settingsversion.Text = '🦦 Otter Client '..mainapi.Version..' '..(
	...
	discordbutton.MouseButton1Click:Connect(function()
		...
		task.spawn(function()
			tooltip.Text = 'Copied!'
			setclipboard('https://discord.gg/nbR79PfPBP')
		end)
	end)
...
function mainapi:Load(skipgui, profile)
	...
	if isfile('newvape/profiles/'..game.GameId..'.gui.txt') then
		...
		if not guidata then
			guidata = {Categories = {}}
			self:CreateNotification('Otter Client', 'Failed to load GUI settings.', 10, 'alert')
			savecheck = false
		end
	...
	if isfile('newvape/profiles/'..self.Profile..self.Place..'.txt') then
		...
		if not savedata then
			...
			self:CreateNotification('Otter Client', 'Failed to load '..self.Profile..' profile.', 10, 'alert')
			savecheck = false
		end
